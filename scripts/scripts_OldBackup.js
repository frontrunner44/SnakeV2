...
const gameDefaults = {
      ...
      bgColor:"rgba(0, 0, 0, 0.85)", // The BG color left behind when your game.snake leaves a position.
...
function eatApple() {
  if(game.apples.indexOf(game.snake[0]) !== -1) { // Checks if the head location is found inside of the game.apples array
    let appleIndex = game.apples.indexOf(game.snake[0]);
    game.score += game.snake.length*2;
    $("#scount").html(game.score);
    game.snake.push(game.addSegmentHere);
    game.apples.splice(appleIndex, 1); // Remove apple from array
    appleTime();
  } 
}
